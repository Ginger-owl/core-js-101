...
function getDigitalRoot(num) {
  const res = String(num).split('').reduce((sum, next) => sum + Number(next), 0);
  if (res < 10) {
    return res;
  }
...
function toNaryString(num, n) {
  ...
  const resultArray = [];
  while (divident > 0) {
    const remainder = divident % n;
    resultArray.unshift(remainder);
    divident = parseInt(divident / n, 10);
  }
...
function getMatrixProduct(m1, m2) {
  ...
  return res.map((row, i) => row.map((_, j) => m1[i].reduce(
    (sum, el, k) => sum + el * m2[k][j], 0,
  )));
}
...
function evaluateTicTacToePosition(position) {
  position.map((row) => {
    if (row.length < 3) {
      for (let i = row.length; i < 3; i += 1) {
        row.push('');
      }
    }
    return row;
  });
  const board = position.reduce((acc, val) => [...acc, ...val], []);
  for (let i = 0; i < 3; i += 1) {
